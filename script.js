...
function displayCities (pastCities) {
    ...
    sortedCities.forEach(function (location){
        ...
        let cityBtn = $('<button>').addClass('search-history-button').text(location.city);
...
function callWeather(queryURL) {
    ...
    $.ajax({
        ...
        weatherIconCon.attr('src', `http://openweathermap.org/img/wn/${weatherIcon}.png`).attr('alt', response.weather[0].description);
        ...
        $.ajax({
            ...
            uvCon.text(uvIndex);
            uvCon.attr('style', `background-color: ${uvColor}; color: ${uvColor === "yellow" ? "black" : "white"}`);
            ...
            for (let i=0; i <= 5; i++) {
                ...
                $(`div.day-${i} .icon`).attr('src', `http://openweathermap.org/img/wn/${today.weather[0].icon}.png`).attr('alt', today.weather[0].description);
                $(`div.day-${i} .days-temp `).text("Temperature: " + ((today.temp.day -273.15) * 1.8 +32).toFixed(1)+ " °F");
                $(`div.day-${i} .days-humid`).text("Humidity " + today.humidity + "%");
            }
        });
    });
}
